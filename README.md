...
located:
```console
sudo tar -xvzf maven-check-1.1.1-bin.tar.gz --directory /opt
export PATH=$PATH:/opt/maven-check-1.1.1/bin
```

#### Windows
Extract the content of the downloaded archive file and move the extracted folder to `C:\maven-check-1.1.1`, then:
...
- Find the "Path" variable, choose "Edit", and add `;C:\maven-check-1.1.1\bin` at the end
