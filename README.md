...
located:
```console
sudo tar -xvzf mvn-check-2.3.0-bin.tar.gz --directory /opt
export PATH=$PATH:/opt/mvn-check-2.3.0/bin
```

#### Windows
Extract the content of the downloaded archive file and move the extracted folder to `C:\mvn-check-2.3.0`, then:
...
- Find the "Path" variable, choose "Edit", and add `;C:\mvn-check-2.3.0\bin` at the end
...
```console
java -jar mvn-check-2.3.0-bin.jar
```
...
- globally in a [XDG-compliant](https://specifications.freedesktop.org/basedir) `mvnchk` directory
