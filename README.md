...
located:
```console
sudo tar -xvzf mvn-check-1.7.2-bin.tar.gz --directory /opt
export PATH=$PATH:/opt/mvn-check-1.7.2/bin
```

#### Windows
Extract the content of the downloaded archive file and move the extracted folder to `C:\mvn-check-1.7.2`, then:
...
- Find the "Path" variable, choose "Edit", and add `;C:\mvn-check-1.7.2\bin` at the end
...
```console
java -jar mvn-check-1.7.2-bin.jar
```
